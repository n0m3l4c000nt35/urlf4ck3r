...
if __name__ == "__main__":
    ...
    while urls_to_visit and not flag.exit():
        ...
        print(f"[{GREEN}CHECKING{END_COLOR}]: {url}")
        ...
        for link in soup.find_all("a"):
            ...
            if href:
                if not scheme:
                    ...
                    if is_internal_url(base_url, href) and href not in visited_urls and href not in urls_to_visit:
    ...
    print(f"[{GREEN}SUBDOMAINS{END_COLOR}]:")
    if len(all_urls["subdomains"]) == 0:
        print(f"[{RED}!{END_COLOR}] No se encontraron subdominios")
    else:
        for url in sorted(all_urls["subdomains"]):
            print(url)
    ...
    if flag.exit():
        print(f"[{RED}!{END_COLOR}] Quedaron por visitar {RED}{len(urls_to_visit)}{END_COLOR} URLs")
        for url in sorted(urls_to_visit):
            print(url)
    elif len(urls_to_visit) == 0:
        print(f"[{GREEN}+{END_COLOR}] Se chequearon todas las URLs posibles")
    else:
